...
if (gene_set == "2000mostvariable"): ## restrict to the 2,000 most variable
	...
	## Clustering using Seurat on whole transcriptome data
	clustering_whole = info_experience.loc[info_experience.index]["seurat_clusters"].astype(str)
else:
	raise ValueError
...
metadata_whole = info_experience.loc[info_experience.index]
...
if (pred_data_type=="whole"):
	samples_whole = np.array([x[:-1] for x in metadata_whole.loc[[m for m in metadata_whole.index if (m in clustering_targeted.index)]]["SAMP"]]) 
	clustering_targeted = clustering_targeted.loc[[m for m in metadata_whole.index if (m in clustering_targeted.index)]]
	clustering_targeted = clustering_targeted.loc[~clustering_targeted.index.duplicated()]
	clustering_whole = clustering_whole[clustering_targeted.index]
	...
	samples_targeted = np.array([metadata.loc[x]["PND"].upper()+"-"+metadata.loc[x]["STIM"].upper() for x in metadata.index])
else:
	raise ValueError
